...
3. 상영시간표 등록시, 영화 가격은 필수 값이다.
4. 상영관의 좌석 수는 제한이 있다.(default 10)
5. 상영 시간표를 등록하기 위해서는 영화 등록이 우선시 되어야 한다.
6. 당일 등록한 영화는, 당일 상영 시간표에 등록할 수 없다.
7. 당일 등록한 영화는, 영화등록 당일 기준 다음날(자정 이후) 부터 상영 시간표에 등록할 수 있다.
8. 상영 시간표를 조회할 수 있다. (일별 / 영화별)
...
2. 할인조건에는 2가지가 존재한다.
   ...
   - 과거일자의 할인조건을 등록할 수 없다.
   - 할인시작 시간 > 할인종료시간 될수 없다.
   ````
   2. ~~기간조건 : 특정 기간 동안 할인을 적용시킨다.~~
   > 2023 4월 17일 ~ 4월 20일 까지 영화할인 가능하다.
3. ~~순서조건과 기간조건은 양립할 수 있으며, 할인율이 가장 큰 할인을 적용시켜 준다.~~ 순서조건만 존재한다.
...
5. 예매상태는 예매완료, 예매취소 두가지 상태가 있다.
6. 예매취소는 전체취소만 가능하다.(3개중 2개만 취소는 불가하다) //추가
